...
export default { getBooks, getBookById, saveReview, addGoogleBook: addBook }

let gBooks = storageService.load('books') || createBooks();
...
function saveReview(bookId, review) {
  let updatedBook = gBooks.find(book => book.id === bookId)

  updatedBook = { ...updatedBook };

  (!updatedBook.reviews) ? updatedBook.reviews = [review] :
    updatedBook.reviews = [...updatedBook.reviews, review]

  gBooks = gBooks.map(book => updatedBook.id === book.id ? updatedBook : book);
...
function createBooks() {
  ...
  storageService.store('books', books)
  return books;
}
